...
### Цель сервиса
Приложение позволяет работодателю через дружелюбный интерфейс сформировать заявку на поиск сотрудника. 
Выбрать одного или команду рекрутеров для поиска лучшего кандидата. В работе сервис использует интеграцию с API HeadHunter.
